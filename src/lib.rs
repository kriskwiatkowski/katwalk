...
pub mod reader {
    ...
    #[derive(Debug, Default)]
    pub struct SignatureWithCtx {
        pub sign: Signature,
        /// Application context, sometimes used by MLDSA, SLH-DSA, ...
        pub ctx: Vec<u8>,
    }
    ...
    // Implement KatParser for SignatureWithCtx. In this flavour, it is the
    // 'ctx' value that is the last one
    impl SignatureWithCtx {
        fn parse_element(self: &mut Self, k: &str, v: &str) -> ReadResult {
            let r = Signature::parse_element(&mut self.sign, k, v);
            match r {
                ReadResult::ReadDone => ReadResult::ReadMore,
                ReadResult::ReadMore => {
                    if k == "ctx" {
                        self.ctx = super::to_u8arr(v);
                        return ReadResult::ReadDone;
                    }
                    return r;
                },
    ...
    impl TestVector {
        ...
        fn parse_element(self: &mut Self, t: AlgType, k: &str, v: &str) -> ReadResult {
            return match t {
                ...
                AlgType::AlgSignatureWithCtx => self.sig_ctx.parse_element(k, v),
...
ctx = 0A0B0C
    ";
        let r = KatReader::new(
            std::io::BufReader::new(Cursor::new(ex)),
            AlgType::AlgSignatureWithCtx,
            1,
        );

        for el in r {
            assert_eq!(el.sig_ctx.ctx, [0x0a, 0x0b, 0x0c]);
            assert_eq!(el.sig_ctx.sign.result, 'T');
        }
    }
}
